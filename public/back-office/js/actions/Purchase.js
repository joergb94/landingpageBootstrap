...
;
    var subtotalLocation = subtotal * belice;
    var totalIvaLocation = subtotalLocation * ivaBelice;
    ...
    $('#total_saletxt').html('$' + total.toFixed(2) + ' '+ 'MX');
    $('#total_sale_blz').val(totalLocation.toFixed(2));
    $('#total_saletxt_blz').html('$' + totalLocation.toFixed(2) + ' '+ 'BZD');
    return total;
  },
...
const purchase = {
  ...
  nextcode: function (id) {
    var sn_id = id + 1;
    const dato = document.getElementById("barcode" + sn_id);
    if (dato) {
      dato.focus();
    }
  },
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if ($('#list' + data.supply_id).length > 0) {
          ...
          var dato = `<tr id="list${data.supply_id}" class="product-purchase">
                                <td>
                                  ${data.name}
                                  <h6>${data.color}</h6>
                                </td>
  ...
  show_supply_son: function () {
    var pieces = $("#pieces").val();
    var price = $("#price").val();
    var name = $("#name").val();
    var name = $("#name").val();
    var color = $("#color").val();
    var price_son = parseFloat(price)/pieces;

    if(pieces > 1 ){
        $("#name_son").val(name);
        $("#color_son").val(color);
        $("#price_son").val(price_son.toFixed(2));
        $("#div_son_supply").show();
    }else{
        $("#name_son").val('');
        $("#color_son").val('');
  ...
  moveAllSN: function (id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        var my_url = url + '/updateAllSerial';
        var type = "POST";
  ...
  label: function (data,pieces) {
    ...
    for (let index = 0; index < data.length; index++) {
      d += `<li class="list-group-item">
              ...
              <label class="form-check-label">
                <input type="checkbox" class="form-check-input serialNumber labelprint" value="${data[index]['sn']}">${data[index]['sn']}
              </label>
            </li>`;
    }
    for (let i = 0; i < pieces.length; i++) {
      d += `<li class="list-group-item">
              <div class="form-check">
              <label class="form-check-label">
                <input type="checkbox" class="form-check-input serialNumber labelprint" value="${pieces[i]['serial_number']}">${pieces[i]['serial_number']} <span class="badge badge-warning">Pza</span>
              </label>
            </li>`;
    }
